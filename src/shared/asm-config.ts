...
function normalizeAsmSharedConfig(raw: unknown): {
	config: AsmSharedConfig | null;
	warnings: string[];
		...
		if (isPlainObject(raw.adapters)) {
			...
			for (const [adapterName, adapterValue] of Object.entries(raw.adapters)) {
				if (!isPlainObject(adapterValue)) {
					warnings.push(
						`adapters.${adapterName} ignored because value is not an object`,
					);
					continue;
				}
	...
	for (const [key, value] of Object.entries(raw)) {
		if (key === "schemaVersion" || key === "core" || key === "adapters")
			continue;
		config[key] = value;
	}
	...
	const invalidShape = warnings.some(
		(item) =>
			item.includes("must be") &&
			(item.startsWith("config root") ||
				item.startsWith("core") ||
				item.startsWith("adapters")),
	);

	return { config, warnings, invalidShape };
}

function resolveAsmConfigPathInfoInternal(
	input: ResolveAsmConfigPathInput = {},
...
export function resolveAsmConfigPath(
	input: ResolveAsmConfigPathInput = {},
): string {
	return resolveAsmConfigPathInfoInternal(input).path;
}

export function resolveAsmConfigPathInfo(
	input: ResolveAsmConfigPathInput = {},
): ResolveAsmConfigPathInfo {
...
export function loadAsmSharedConfig(
	input: LoadAsmSharedConfigInput = {},
): LoadAsmSharedConfigResult {
	...
	try {
		...
		const status = message.toLowerCase().includes("json")
			? "invalid_json"
			: "read_error";
		return {
			path,
...
function resolveLegacySlotDbDir(
	config: AsmSharedConfig | null,
): string | undefined {
	const coreStorage = (config?.core as Record<string, unknown> | undefined)
		?.storage as Record<string, unknown> | undefined;
	return firstNonEmptyString(
		config?.core?.slotDbDir,
		coreStorage?.slotDbDir,
		(config?.storage as Record<string, unknown> | undefined)?.slotDbDir,
		config?.slotDbDir,
	);
}

function resolveLegacyWikiDir(
	config: AsmSharedConfig | null,
): string | undefined {
	return firstNonEmptyString(config?.core?.wikiDir, config?.wikiDir);
}

export function resolveAsmCoreSlotDbDir(
	input: LoadAsmSharedConfigInput = {},
): string | undefined {
	const { config } = loadAsmSharedConfig(input);
	const value = resolveLegacySlotDbDir(config);
	return value
		? expandHome(value, input.homeDir || input.env?.HOME)
		: undefined;
}

export function resolveAsmCoreWikiDir(
	input: LoadAsmSharedConfigInput = {},
): string | undefined {
	const { config } = loadAsmSharedConfig(input);
	const value = resolveLegacyWikiDir(config);
	return value
		? expandHome(value, input.homeDir || input.env?.HOME)
		: undefined;
}

export function resolveAsmCoreProjectWorkspaceRoot(
	input: LoadAsmSharedConfigInput = {},
): string | undefined {
	const { config } = loadAsmSharedConfig(input);
...
export function resolveAsmCoreConfigValue<T = unknown>(
	key: keyof AsmSharedConfigCore,
	input: LoadAsmSharedConfigInput = {},
): T | undefined {
	const { config } = loadAsmSharedConfig(input);
...
export function getAsmSharedConfig(
	input: LoadAsmSharedConfigInput = {},
): LoadAsmSharedConfigResult {
...
export function resolveAsmRuntimeConfig(
	input: LoadAsmSharedConfigInput = {},
): AsmResolvedRuntimeConfig {
	...
	const wikiDir = resolveAsmCoreWikiDir(input);
	const projectWorkspaceRoot = resolveAsmCoreProjectWorkspaceRoot(input);
	if (!slotDbDir)
		throw new Error(
			`ASM shared config missing required field: core.slotDbDir at ${path}`,
		);
	if (!wikiDir)
		throw new Error(
			`ASM shared config missing required field: core.wikiDir at ${path}`,
		);
	if (!projectWorkspaceRoot)
		throw new Error(
			`ASM shared config missing required field: core.projectWorkspaceRoot at ${path}`,
		);
	return {
		asmConfigPath: path,
...
export function doctorAsmSharedConfig(
	input: LoadAsmSharedConfigInput = {},
): AsmSharedConfigDoctorResult {
	...
	return {
		...
		adapterNames: Object.keys(loaded.config?.adapters || {}).sort((a, b) =>
			a.localeCompare(b),
		),
		legacyKeys: {
			slotDbDir: typeof loaded.config?.slotDbDir === "string",
			projectWorkspaceRoot:
				typeof loaded.config?.projectWorkspaceRoot === "string",
			storageSlotDbDir:
				typeof (loaded.config?.storage as Record<string, unknown> | undefined)
					?.slotDbDir === "string",
		},
