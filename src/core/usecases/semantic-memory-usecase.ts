...
import { getAgentNamespaces, getNamespaceWeight, parseExplicitNamespace, toCoreAgent, type MemoryNamespace } from "../../shared/memory-config.js";
...
export class SemanticMemoryUseCase {
  ...
  async capture(payload: MemoryCapturePayload, context: MemoryContext): Promise<MemoryCaptureResult> {
  ...
  async search(payload: MemorySearchPayload, context: MemoryContext): Promise<MemorySearchResult> {
    ...
    if (namespaceFilter) {
      ...
      filterMust.push({ should: namespaces.map((ns) => ({ key: "namespace", match: { value: ns } })) });
    }
    ...
    if (payload.userId || context.userId) {
      filterMust.push({ key: "userId", match: { value: payload.userId || context.userId } });
    }

    if (payload.sourceAgent) {
      filterMust.push({ key: "source_agent", match: { value: payload.sourceAgent } });
    }

    const vector = await this.embedding.embed(query);
    const points = await this.qdrant.search(vector, limit, { must: filterMust });
    ...
    const weighted = points
      ...
      .map((r: ScoredPoint) => {
        ...
        const weightedScore = Math.min(1, r.score * weight);
        return {
          id: String(r.id),
          rawScore: r.score,
          score: weightedScore,
          ...
          namespace: ns,
          timestamp: typeof r.payload?.timestamp === "number" ? r.payload.timestamp : undefined,
  ...
  private resolveNamespace(namespace: string | undefined, sourceAgent: string): MemoryNamespace {
  ...
  private async embedDetailedCompat(text: string): Promise<{ vector: number[]; metadata: Record<string, unknown> }> {
