...
function normalizePrivateIdentity(ctx: {
	userId: string;
	agentId: string;
}): ScopeIdentity {
	return {
...
function allScopeIdentities(ctx: {
	userId: string;
	agentId: string;
}): ScopeIdentity[] {
	const base = normalizePrivateIdentity(ctx);
...
export class DefaultMemoryUseCasePort implements MemoryUseCasePort {
	...
	async run<TReq, TRes>(
		...
		switch (useCase) {
			case "slot.get":
				return this.handleSlotGet(
					payload as unknown as SlotGetPayload,
					req,
				) as TRes;
			case "slot.set":
				return this.handleSlotSet(
					payload as unknown as SlotSetPayload,
					req,
				) as TRes;
			case "slot.list":
				return this.handleSlotList(
					payload as unknown as SlotListPayload,
					req,
				) as TRes;
			case "slot.delete":
				return this.handleSlotDelete(
					payload as unknown as SlotDeletePayload,
					req,
				) as TRes;
			case "project.register":
				return this.handleProjectRegister(
					payload as unknown as ProjectRegisterPayload,
					req,
				) as TRes;
			case "project.get":
				return this.handleProjectGet(
					payload as unknown as ProjectGetPayload,
					req,
				) as TRes;
			case "project.binding_preview":
				return this.handleProjectBindingPreview(
					payload as unknown as ProjectBindingPreviewPayload,
					req,
				) as TRes;
			case "project.opencode_search":
				return this.handleProjectOpenCodeSearch(
					payload as unknown as ProjectOpenCodeSearchPayload,
					req,
				) as TRes;
			case "project.list":
				return this.handleProjectList(req) as TRes;
			case "project.set_registration_state":
				return this.handleProjectSetRegistrationState(
					payload as unknown as ProjectSetRegistrationStatePayload,
					req,
				) as TRes;
			case "project.set_tracker_mapping":
				return this.handleProjectSetTrackerMapping(
					payload as unknown as ProjectSetTrackerMappingPayload,
					req,
				) as TRes;
			case "project.register_command":
				return this.handleProjectRegisterCommand(
					payload as unknown as ProjectRegisterCommandPayload,
					req,
				) as TRes;
			case "project.link_tracker":
				return this.handleProjectLinkTracker(
					payload as unknown as ProjectLinkTrackerPayload,
					req,
				) as TRes;
			case "project.trigger_index":
				return this.handleProjectTriggerIndex(
					payload as unknown as ProjectTriggerIndexPayload,
					req,
				) as TRes;
			case "project.deindex":
				return this.handleProjectDeindex(
					payload as unknown as ProjectDeindexPayload,
					req,
				) as TRes;
			case "project.detach":
				return this.handleProjectDetach(
					payload as unknown as ProjectDetachPayload,
					req,
				) as TRes;
			case "project.unregister":
				return this.handleProjectUnregister(
					payload as unknown as ProjectUnregisterPayload,
					req,
				) as TRes;
			case "project.purge_preview":
				return this.handleProjectPurgePreview(
					payload as unknown as ProjectPurgePreviewPayload,
					req,
				) as TRes;
			case "project.purge":
				return this.handleProjectPurge(
					payload as unknown as ProjectPurgePayload,
					req,
				) as TRes;
			case "project.reindex_diff":
				return this.handleProjectReindexDiff(
					payload as unknown as ProjectReindexDiffPayload,
					req,
				) as TRes;
			case "project.index_event":
				return this.handleProjectIndexEvent(
					payload as unknown as ProjectIndexEventPayload,
					req,
				) as TRes;
			case "project.install_hooks":
				return this.handleProjectInstallHooks(
					payload as unknown as ProjectInstallHooksPayload,
					req,
				) as TRes;
			case "project.index_watch_get":
				return this.handleProjectIndexWatchGet(
					payload as unknown as ProjectIndexWatchGetPayload,
					req,
				) as TRes;
			case "project.task_registry_upsert":
				return this.handleProjectTaskRegistryUpsert(
					payload as unknown as ProjectTaskRegistryUpsertPayload,
					req,
				) as TRes;
			case "project.task_lineage_context":
				return this.handleProjectTaskLineageContext(
					payload as unknown as ProjectTaskLineageContextPayload,
					req,
				) as TRes;
			case "project.hybrid_search":
				return this.handleProjectHybridSearch(
					payload as unknown as ProjectHybridSearchPayload,
					req,
				) as TRes;
			case "project.change_overlay.query":
				return this.handleProjectChangeOverlayQuery(
					payload as unknown as ProjectChangeOverlayQueryUseCasePayload,
					req,
				) as TRes;
			case "project.legacy_backfill":
				return this.handleProjectLegacyBackfill(
					payload as unknown as ProjectLegacyBackfillPayload,
					req,
				) as TRes;
			case "project.telegram_onboarding":
				return this.handleProjectTelegramOnboarding(
					payload as unknown as ProjectTelegramOnboardingPayload,
					req,
				) as TRes;
			case "project.feature_pack.generate":
				return this.handleProjectFeaturePackGenerate(
					payload as unknown as ProjectFeaturePackGeneratePayload,
					req,
				) as TRes;
			case "project.feature_pack.query":
				return this.handleProjectFeaturePackQuery(
					payload as unknown as ProjectFeaturePackQueryPayload,
					req,
				) as TRes;
			case "project.developer_query":
				return this.handleProjectDeveloperQuery(
					payload as unknown as ProjectDeveloperQueryPayload,
					req,
				) as TRes;
			case "project.routing_contract":
				return this.handleProjectRoutingContract(
					payload as unknown as ProjectRoutingContractPayload,
					req,
				) as TRes;
			case "project.coding_packet":
				return this.handleProjectCodingPacket(
					payload as unknown as ProjectCodingPacketPayload,
					req,
				) as TRes;
			case "graph.entity.get":
				return this.handleGraphEntityGet(
					payload as unknown as GraphEntityGetPayload,
					req,
				) as TRes;
			case "graph.entity.set":
				return this.handleGraphEntitySet(
					payload as unknown as GraphEntitySetPayload,
					req,
				) as TRes;
			case "graph.rel.add":
				return this.handleGraphRelAdd(
					payload as unknown as GraphRelAddPayload,
					req,
				) as TRes;
			case "graph.rel.remove":
				return this.handleGraphRelRemove(
					payload as unknown as GraphRelRemovePayload,
					req,
				) as TRes;
			case "graph.search":
				return this.handleGraphSearch(
					payload as unknown as GraphSearchPayload,
					req,
				) as TRes;
			case "graph.code.upsert":
				return this.handleGraphCodeUpsert(
					payload as unknown as GraphCodeUpsertPayload,
					req,
				) as TRes;
			case "graph.code.chain":
				return this.handleGraphCodeChain(
					payload as unknown as GraphCodeChainPayload,
					req,
				) as TRes;
			case "memory.capture":
	...
	private handleSlotList(
		...
		if (payload.scope === "all" || !payload.scope) {
			return allScopeIdentities(req.context).flatMap((identity) =>
				this.slotDb
					.list(identity.userId, identity.agentId, {
						category: payload.category,
	...
	private handleProjectGet(
		...
		if (!payload.project_id && !payload.project_alias) {
			throw new Error(
				"project.get requires payload.project_id or payload.project_alias",
			);
		}

		if (payload.project_id) {
			const project = this.slotDb.getProjectById(
				identity.userId,
				identity.agentId,
				payload.project_id,
			);
			...
			return {
				project,
				registration: this.slotDb.getProjectRegistrationState(
					identity.userId,
					identity.agentId,
					payload.project_id,
				),
		...
		const byAlias = this.slotDb.getProjectByAlias(
			identity.userId,
			identity.agentId,
	...
	private handleProjectBindingPreview(
		payload: ProjectBindingPreviewPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		const projects = this.slotDb.listProjects(
			identity.userId,
			identity.agentId,
		);
		const normalizedRepoRoot = this.normalizeRepoRootInput(payload.repo_root);
		const aliasSelector = String(
			payload.project_alias || payload.session_project_alias || "",
		).trim();
		const matches: Array<{
			project_id: string;
		...
		const pushMatch = (
			...
			if (
				matches.some(
					(item) =>
						item.project_id === project.project_id && item.source === source,
				)
			)
				return;
			matches.push({
				...
				project_alias:
					aliases.find((item) => item.is_primary === 1)?.project_alias ||
					aliases[0]?.project_alias ||
					null,
		...
		if (payload.project_id) {
			const direct = this.slotDb.getProjectById(
				identity.userId,
				identity.agentId,
				payload.project_id,
			);
			if (direct) {
				const row = projects.find(
					(item) => item.project.project_id === direct.project_id,
				);
		...
		if (aliasSelector) {
			...
			if (byAlias) {
				const row = projects.find(
					(item) => item.project.project_id === byAlias.project.project_id,
				);
				pushMatch(
					byAlias.project,
					row?.aliases || [byAlias.alias],
					payload.project_alias ? "project_alias" : "session_project_alias",
				);
			}
		}
		...
		if (normalizedRepoRoot) {
			for (const row of projects) {
				const projectRepoRoot = this.normalizeRepoRootInput(
					row.project.repo_root || undefined,
				);
		...
		const uniqueByProject = Array.from(
			new Map(matches.map((item) => [item.project_id, item] as const)).values(),
		);
		const activeMatches = uniqueByProject.filter(
			(item) => item.lifecycle_status === "active",
		);
		const crossProjectRequired = activeMatches.length > 1;
		const allowed =
			!crossProjectRequired || payload.allow_cross_project === true;
		const selected = activeMatches[0] || uniqueByProject[0] || null;

		const resolutionStatus = selected
			? crossProjectRequired && !allowed
				? "ambiguous"
				: "resolved"
			: "unresolved";
		...
		return {
			...
			resolution: {
				...
				read_only_tool_surface: [
					...
					"project_developer_query",
				],
			},
			errors: selected
				? crossProjectRequired && !allowed
					? [
							"multiple active project matches found; explicit cross-project approval required",
						]
					: []
				: [
						normalizedRepoRoot
							? "no active registered project matched repo_root"
							: "no registered project matched provided selectors",
					],
		};
	}

	private handleProjectOpenCodeSearch(
		payload: ProjectOpenCodeSearchPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const binding = this.handleProjectBindingPreview(
			{
				...
				allow_cross_project:
					payload.explicit_cross_project || payload.allow_cross_project,
			},
			req,
		) as any;

		if (
			binding.resolution_status !== "resolved" ||
			!binding.selected_project?.project_id
		) {
			return {
				mode: "read-only",
				...
				errors: binding.errors || [
					"project binding could not be resolved for read-only search",
				],
			};
		}
		...
		const selectedLifecycle = String(
			binding.selected_project.lifecycle_status || "active",
		);

		if (
			["deindexed", "detached", "disabled", "purged"].includes(
				selectedLifecycle,
			)
		) {
			return {
	...
	private normalizeWorkstreamType(raw?: string): ProjectWorkstreamType {
		const normalized = String(raw || "")
			...
			.toLowerCase();
		if (
			normalized === "research_execution" ||
			normalized === "coding_execution" ||
	...
	private handleProjectRoutingContract(
		...
		const workstreamType = this.normalizeWorkstreamType(
			payload.workstream_type,
		);
		const reasonByWorkstream: Record<ProjectWorkstreamType, string> = {
			coding_execution:
				"OpenClaw orchestrates and routes coding execution to OpenCode lane foundation.",
			research_execution:
				"Research execution keeps OpenCode lane ready through project/code-aware retrieval foundation.",
			review_execution:
				"Review execution can reuse OpenCode coding lane foundation with project-aware packet context.",
		};
	...
	private handleProjectCodingPacket(
		...
		const query = String(
			payload.query || payload.objective || payload.task_title || "",
		).trim();
		const objective =
			String(payload.objective || payload.query || "").trim() ||
			"Implement requested coding change with project-aware/code-aware context.";
	...
	private handleProjectSetRegistrationState(
		payload: ProjectSetRegistrationStatePayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (!payload.project_id) {
			throw new Error(
				"project.set_registration_state requires payload.project_id",
			);
		}

		return this.slotDb.updateProjectRegistrationState(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
	...
	private handleProjectSetTrackerMapping(
		payload: ProjectSetTrackerMappingPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (!payload.project_id || !payload.tracker_type) {
			throw new Error(
				"project.set_tracker_mapping requires payload.project_id and payload.tracker_type",
			);
		}

		if (payload.tracker_type === "jira") {
			this.validateJiraTrackerFields(
				payload.tracker_space_key,
				payload.default_epic_key,
			);
		}

		return this.slotDb.setProjectTrackerMapping(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
	...
	private handleProjectRegisterCommand(
		payload: ProjectRegisterCommandPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (!alias) {
			throw new Error(
				"project.register_command requires payload.project_alias",
			);
		}

		const repoUrl =
			String(payload.repo_url || payload.repo_remote || "").trim() || undefined;
		...
		const resolvedRepoRemote =
			payload.repo_remote || selection.repo_remote || repoUrl;

		const registered = this.slotDb.registerProject(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
				...
				reuse_existing_repo_root:
					selection.resolution === "registered_remote_match",
			},
		);
		...
		if (payload.tracker?.tracker_type) {
			if (payload.tracker.tracker_type === "jira") {
				this.validateJiraTrackerFields(
					payload.tracker.tracker_space_key,
					payload.tracker.default_epic_key,
				);
			}

			trackerMapping = this.slotDb.setProjectTrackerMapping(
				identity.userId,
				identity.agentId,
				{
					...
					active_version:
						payload.tracker.active_version || payload.active_version,
		...
		const autoIndexHook = this.installProjectGitHooks(
			registered.project.project_id,
			registered.project.repo_root,
		);
		...
		return {
			...
			completeness_score: Number(
				(registered.registration.completeness_score / 100).toFixed(2),
			),
			warnings: [],
	...
	private handleProjectLinkTracker(
		payload: ProjectLinkTrackerPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (mode !== "attach_or_update") {
			throw new Error(
				"project.link_tracker only supports mode=attach_or_update",
			);
		}

		const project = this.resolveProjectRef(
			identity.userId,
			identity.agentId,
			payload.project_ref,
		);
		...
		const mapped = this.slotDb.setProjectTrackerMapping(
			identity.userId,
			identity.agentId,
			{
				project_id: project.project_id,
	...
	private handleProjectTriggerIndex(
		payload: ProjectTriggerIndexPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		const project = this.resolveProjectRef(
		...
		const normalizedPaths = this.hydrateIndexPathsFromRepo(
			...
			(payload.paths || []).filter(
				(item) => String(item.relative_path || "").trim().length > 0,
			),
		);
	...
	private scheduleProjectReindexJob(input: {
		...
		setTimeout(() => {
			try {
				...
				this.slotDb.reindexProjectByDiff(
					input.scopeUserId,
					input.scopeAgentId,
					{
	...
	private resolveProjectRef(
		...
		if (projectRef?.project_id) {
			...
			if (!project)
				throw new Error(
					`project_id '${projectRef.project_id}' is not registered`,
				);
			return project;
		}
		...
		if (projectRef?.project_alias) {
			const byAlias = this.slotDb.getProjectByAlias(
				scopeUserId,
				scopeAgentId,
				projectRef.project_alias,
			);
			if (!byAlias)
				throw new Error(
					`project_alias '${projectRef.project_alias}' is not registered`,
				);
	...
	): { installed: boolean; hooks: string[]; note?: string } {
		if (!repoRoot)
...
`;
		...
		const attachHook = (
			name: string,
			eventType: "post_commit" | "post_merge" | "post_rewrite",
		) => {
	...
	private validateJiraTrackerFields(
		...
		if (epic) {
			...
			if (!epic.toUpperCase().startsWith(expectedPrefix)) {
				throw new Error(
					`default_epic_key must match tracker_space_key prefix '${space}-*'`,
				);
			}
		}
	}
	...
	private resolveWorkspaceRoot(req: CoreRequestEnvelope<unknown>): string {
		...
		const sharedWorkspaceRoot = resolveAsmCoreProjectWorkspaceRoot({
			env: process.env,
	...
	private resolveRepoForRegistration(
		...
		const explicitRepoRoot = this.normalizeRepoRootInput(
			input.explicitRepoRoot,
		);
		if (explicitRepoRoot) {
		...
		if (cwdRoot) {
			...
			const cwdMatchesRepoUrl =
				!normalizedRepoUrl ||
				!cwdRemote ||
				this.canonicalizeRemote(cwdRemote) ===
					this.canonicalizeRemote(normalizedRepoUrl);
			...
			notes.push(
				"current git root exists but remote does not match repo_url; skipped cwd reuse",
			);
		}

		if (normalizedRepoUrl) {
			const registered = this.findRegisteredProjectByRemote(
				scopeUserId,
				scopeAgentId,
				normalizedRepoUrl,
			);
			...
			const imported = this.tryResolveLocalPathImport(
				normalizedRepoUrl,
				input.workspaceRoot,
			);
			if (imported) {
				notes.push(
					"repo_url points to a local path; imported without git clone",
				);
				return {
					repo_root: imported,
			...
			const cloneAttempt = this.cloneOrReuseRepo(
				normalizedRepoUrl,
				input.workspaceRoot,
			);
		...
		notes.push(
			"repo root unresolved: no explicit repo_root, not inside git repo, and no repo_url provided",
		);
		return {
			repo_root: undefined,
	...
	private tryResolveLocalPathImport(
		...
		const rawPath = repoUrl.startsWith("file://")
	...
	private cloneOrReuseRepo(
		...
		if (this.isMatchingExistingClone(preferredTarget, repoUrl)) {
			notes.push(
				`existing clone already present at ${preferredTarget}; reused`,
			);
			return {
		...
		const conflictTarget = this.computeConflictCloneTarget(
			preferredTarget,
			repoUrl,
		);
		this.gitClone(repoUrl, conflictTarget);
		notes.push(
			`preferred clone target occupied; cloned with suffix at ${conflictTarget}`,
		);
		return {
	...
	private isMatchingExistingClone(targetDir: string, repoUrl: string): boolean {
		...
		return (
			this.canonicalizeRemote(existingRemote) ===
			this.canonicalizeRemote(repoUrl)
		);
	}
		...
		try {
			...
			return files.map((relativePath) => {
				...
				return {
					...
					module: relativePath.split("/")[0] || undefined,
					language: relativePath.includes(".")
						? relativePath.split(".").pop() || undefined
						: undefined,
					content,
				};
			});
	...
	private handleProjectDeindex(
		payload: ProjectDeindexPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
	...
	private handleProjectDetach(
		payload: ProjectDetachPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		const project = this.resolveProjectRef(
	...
	private handleProjectUnregister(
		payload: ProjectUnregisterPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		const project = this.resolveProjectRef(
	...
	private handleProjectPurgePreview(
		payload: ProjectPurgePreviewPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		const project = this.resolveProjectRef(
	...
	private handleProjectPurge(
		payload: ProjectPurgePayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		const project = this.resolveProjectRef(
	...
	private handleProjectReindexDiff(
		payload: ProjectReindexDiffPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
	...
	private handleProjectIndexEvent(
		payload: ProjectIndexEventPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		if (!payload.project_id) {
		...
		if (!project || !project.repo_root) {
			throw new Error(
				"project.index_event requires a registered project with repo_root",
			);
		}

		const registeredRepoRoot = this.normalizeRepoRootInput(
			project.repo_root || undefined,
		);
		const eventRepoRoot = this.normalizeRepoRootInput(
			payload.repo_root || undefined,
		);
		if (
			eventRepoRoot &&
			registeredRepoRoot &&
			eventRepoRoot !== registeredRepoRoot
		) {
			throw new Error(
				`project.index_event repo_root mismatch: event='${eventRepoRoot}' registered='${registeredRepoRoot}'`,
			);
		}
		...
		const paths = changedFiles.map((relativePath) => {
			...
			return {
				...
				checksum:
					content != null && String(content).trim() !== ""
						? sha1(String(content))
						: `event:${relativePath}:${payload.source_rev || "unknown"}`,
				module: relativePath.split("/")[0] || undefined,
				language: relativePath.includes(".")
					? relativePath.split(".").pop() || undefined
					: undefined,
		...
		const reindex = this.slotDb.reindexProjectByDiff(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
		...
		if (deletedFiles.length > 0) {
			...
			for (const relativePath of deletedFiles) {
				this.slotDb.markProjectFileDeletedForEvent(
					identity.userId,
					identity.agentId,
					payload.project_id,
					relativePath,
	...
	private handleProjectInstallHooks(
		payload: ProjectInstallHooksPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		if (!payload.project_id) {
	...
	private handleProjectIndexWatchGet(
		payload: ProjectIndexWatchGetPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		return this.slotDb.getProjectIndexWatchState(
			identity.userId,
			identity.agentId,
			payload.project_id,
		);
	...
	private handleProjectTaskRegistryUpsert(
		payload: ProjectTaskRegistryUpsertPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (!payload.task_id || !payload.project_id || !payload.task_title) {
			throw new Error(
				"project.task_registry_upsert requires payload.task_id, payload.project_id, payload.task_title",
			);
		}

		return this.slotDb.upsertTaskRegistryRecord(
			identity.userId,
			identity.agentId,
			{
				task_id: payload.task_id,
	...
	private handleProjectTaskLineageContext(
		payload: ProjectTaskLineageContextPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (!payload.project_id) {
			throw new Error(
				"project.task_lineage_context requires payload.project_id",
			);
		}

		if (!payload.task_id && !payload.tracker_issue_key && !payload.task_title) {
			throw new Error(
				"project.task_lineage_context requires one selector: task_id|tracker_issue_key|task_title",
			);
		}

		return this.slotDb.getTaskLineageContext(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
	...
	private handleProjectHybridSearch(
		payload: ProjectHybridSearchPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		if (!payload.project_id || !payload.query) {
			throw new Error(
				"project.hybrid_search requires payload.project_id and payload.query",
			);
		}

		return this.slotDb.hybridSearchProjectContext(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
	...
	private handleProjectChangeOverlayQuery(
		...
		if (!payload.project_id) {
			throw new Error(
				"project.change_overlay.query requires payload.project_id",
			);
		}
		if (!payload.task_id && !payload.tracker_issue_key && !payload.task_title) {
			throw new Error(
				"project.change_overlay.query requires one selector: task_id|tracker_issue_key|task_title",
			);
		}
		...
		const requestedFeature = payload.feature_key
			...
			: payload.feature_name
				? this.resolveFeatureKeyInput(undefined, payload.feature_name)
				: undefined;

		const result = this.slotDb.queryProjectChangeOverlay(
			identity.userId,
			identity.agentId,
			{
				project_id: payload.project_id,
		...
		const baseEvidence: ProjectChangeOverlayEvidenceItem[] = [
			{
				type: "task",
				ref: result.focus.task_id,
				note: result.focus.task_title,
			},
			...(result.focus.tracker_issue_key
				? [
						{
							type: "tracker_issue" as const,
							ref: result.focus.tracker_issue_key,
						},
					]
				: []),
		...
		for (const featureKey of featurePackCandidates) {
			...
			const packEvidenceSet = new Set(
				(pack.evidence || []).map(
					(item) => `${item.type}:${String(item.ref || "").toLowerCase()}`,
				),
			);
			const matchedEvidence = baseEvidence.filter((item) =>
				packEvidenceSet.has(
					`${item.type}:${String(item.ref || "").toLowerCase()}`,
				),
			);
			...
			const trackerHit =
				...
				pack.evidence.some(
					(item) =>
						item.type === "task" &&
						String(item.ref || "") ===
							String(result.focus.tracker_issue_key || ""),
				);
			const taskHit = pack.evidence.some(
				(item) =>
					item.type === "task" &&
					String(item.ref || "") === String(result.focus.task_id),
			);
			const commitHitCount = result.commit_refs.filter((ref) =>
				pack.related_commits.some((hint) =>
					hint.toLowerCase().includes(String(ref || "").toLowerCase()),
				),
			).length;

			const overlapRatio =
				matchedEvidence.length / Math.max(baseEvidence.length, 1);
			const confidenceRaw =
		...
		for (const symbol of result.related_symbols) {
			if (symbol.relative_path && symbolsByPath.has(symbol.relative_path)) {
				symbolsByPath.set(
					...
					Number(symbolsByPath.get(symbol.relative_path) || 0) + 1,
				);
			}
		}
		...
		const enrichedSymbols = result.related_symbols
			.map((symbol) => {
				const hasPath = Boolean(
					symbol.relative_path &&
						result.changed_files.includes(symbol.relative_path),
				);
				const pathDensity = hasPath
					? Math.min(
							1,
							Number(symbolsByPath.get(symbol.relative_path || "") || 0) / 4,
						)
					: 0;
				...
				return {
					...
					evidence_refs: [
						...
						...(symbol.symbol_fqn
							? [`symbol:${symbol.symbol_fqn}`]
							: [`symbol:${symbol.symbol_name}`]),
					],
				};
			})
			.sort((a, b) => {
				const confidenceDiff = Number(
	...
	private handleProjectLegacyBackfill(
		payload: ProjectLegacyBackfillPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);

		return this.slotDb.runLegacyCompatibilityBackfill(
			identity.userId,
			identity.agentId,
			{
	...
	private handleProjectTelegramOnboarding(
		...
		const workspaceRoot =
			this.normalizeRepoRootInput(payload.project_workspace_root) ||
			this.resolveWorkspaceRoot(req);
		const draft = {
			...
			jira_space_key: String(payload.jira_space_key || "")
				.trim()
				.toUpperCase(),
			default_epic_key: String(payload.default_epic_key || "")
				.trim()
				.toUpperCase(),
		...
		if (!draft.project_alias) {
			warnings.push(
				"project_alias is empty; alias should be confirmed before final commit",
			);
		}
		...
		if (draft.jira_space_key) {
			...
			if (draft.default_epic_key) {
				...
				if (!draft.default_epic_key.startsWith(expectedPrefix)) {
					errors.push(
						`default_epic_key must match jira_space_key prefix '${draft.jira_space_key}-*'`,
					);
				}
			}
		} else if (draft.default_epic_key) {
			errors.push(
				"jira_space_key is required when default_epic_key is provided",
			);
		}
	...
	private handleProjectFeaturePackGenerate(
		...
		const identity = normalizePrivateIdentity(req.context);
		const featureKey =
			payload.feature_key || "project_onboarding_registration_indexing";
		...
		const snapshot =
			this.slotDb.getProjectFeaturePackProjectOnboardingIndexingSnapshot(
				identity.userId,
				identity.agentId,
				project.project_id,
			);
		...
		const primaryAlias =
			snapshot.aliases.find((item) => item.is_primary === 1)?.project_alias ||
			payload.project_alias ||
			project.project_name;

		switch (featureKey) {
			case "project_onboarding_registration_indexing":
				return this.buildProjectOnboardingRegistrationIndexingPack(
					snapshot,
					project.project_id,
					primaryAlias,
				);
			case "code_aware_retrieval":
				return this.buildCodeAwareRetrievalPack(
					snapshot,
					project.project_id,
					primaryAlias,
				);
			case "heartbeat_health_runtime_integrity":
				return this.buildHeartbeatHealthRuntimeIntegrityPack(
					snapshot,
					project.project_id,
					primaryAlias,
				);
			case "change_aware_impact":
				return this.buildChangeAwareImpactPack(
					snapshot,
					project.project_id,
					primaryAlias,
				);
			case "post_entry_review_decision_support":
				return this.buildPostEntryReviewDecisionSupportPack(
					snapshot,
					project.project_id,
					primaryAlias,
				);
	...
	private handleProjectFeaturePackQuery(
		...
		const requestedFeature = this.resolveFeatureKeyInput(
			payload.feature_key,
			payload.feature_name,
		);
	...
	private handleProjectDeveloperQuery(
		...
		const retrievalPlan = this.buildDeveloperRetrievalPlan(
			parsed.canonical_intent,
			payload.limit,
			parsed,
		);
		...
		const queryFingerprint = createHash("sha1")
			.update(
				`${project.project_id}|${parsed.canonical_intent}|${query.toLowerCase()}`,
			)
			.digest("hex")
			.slice(0, 16);
		const queryId = `pdevq:${queryFingerprint}`;
		const trackerIssueHint =
			parsed.tracker_issue_key ||
			parsed.tracker_issue_keys?.[0] ||
			query.match(/\b[A-Z][A-Z0-9_]+-\d+\b/)?.[0];
		...
		let locate = this.handleProjectHybridSearch(
			{
				...
				...(retrievalPlan.path_prefix_hint &&
				retrievalPlan.path_prefix_hint.length > 0
		...
		const sourceRank = retrievalPlan.source_priority.reduce<
			Record<DeveloperRetrievalSource, number>
		>(
			(acc, source, index) => {
				acc[source] = index;
				return acc;
		...
		const sourceBoostByPriority = retrievalPlan.source_priority.reduce<
			Record<DeveloperRetrievalSource, number>
		>(
			(acc, source, index) => {
		...
		const sortStringsStable = (values: string[]) =>
		...
		const locateScored = locate.results.map((item) => {
			...
			const adjustedScore = Number(
				(
					Number(item.score || 0) + (sourceBoostByPriority[source] || 0)
				).toFixed(4),
			);
			return {
				...item,
		...
		const locateSorted = [...locateScored].sort(
			(a, b) =>
				Number(b.adjusted_score || 0) - Number(a.adjusted_score || 0) ||
				(sourceRank[a.source as DeveloperRetrievalSource] ?? 99) -
					(sourceRank[b.source as DeveloperRetrievalSource] ?? 99) ||
				String(a.relative_path || "").localeCompare(
					String(b.relative_path || ""),
				) ||
				String(a.symbol_name || "").localeCompare(
					String(b.symbol_name || ""),
				) ||
				String(a.task_id || a.id).localeCompare(String(b.task_id || b.id)),
		);

		const locatePrimary: ProjectDeveloperQueryPrimaryResult[] =
			locateSorted.map((item) => ({
				type:
		...
		const shouldAttachFeaturePack =
			...
			Boolean(
				parsed.feature_key || payload.feature_key || payload.feature_name,
			);
		...
		const overlaySelector = this.pickOverlaySelectorFromLocate(
			locate.results,
			parsed,
		);
		let overlay: ProjectChangeOverlayV1 | null = null;
		if (
			retrievalPlan.attach_overlay &&
			(overlaySelector.task_id || overlaySelector.tracker_issue_key)
		) {
			try {
		...
		const featurePrimary: ProjectDeveloperQueryPrimaryResult[] =
			retrievalPlan.plan_key === "feature_lookup"
				? featurePacks
						.map(
							(pack): ProjectDeveloperQueryPrimaryResult => ({
								type: "feature_pack",
								id: pack.pack_id,
								title: pack.title,
								score: 1,
		...
		const mergedPrimaryCandidates = [
			...(retrievalPlan.plan_key === "feature_lookup" &&
			retrievalPlan.prefer_feature_primary
				? featurePrimary
				: []),
		...
		const symbols = sortStringsStable(
			Array.from(
				new Set([
					...(locateSorted
						...
						.filter(Boolean) as string[]),
					...((overlay?.related_symbols || [])
						.map((item) => item.symbol_fqn || item.symbol_name)
						.filter(Boolean) as string[]),
				]),
			),
		).slice(0, topN.symbols);

		const snippets = sortStringsStable(
			Array.from(
				new Set([
					...locateSorted
					...
					...(overlay
						? [
								...
								...overlay.feature_packs
									.slice(0, 2)
									.map(
										(pack) =>
											`${pack.feature_key}: ${pack.note || "overlay match"}`,
									),
							]
						: []),
				]),
		...
		const graphPaths = sortStringsStable(
			overlay
				? Array.from(
						new Set(
							overlay.related_symbols.map(
								(item) =>
									`${item.relative_path || "unknown"}::${item.symbol_fqn || item.symbol_name}`,
							),
						),
					)
				: [],
		...
		const changeContext = sortStringsStable(
			Array.from(
				new Set([
					...locateSorted
					...
					...(overlay
						? [
								overlay.focus.task_id,
								...(overlay.focus.tracker_issue_key
									? [overlay.focus.tracker_issue_key]
									: []),
							]
						: []),
		...
		const whyThisResult = [
			...
			...(featurePacks.length > 0
				? ["enriched via project.feature_pack.query"]
				: []),
		...
		const answerTemplate =
			parsed.canonical_intent === "locate_symbol" ||
			parsed.canonical_intent === "locate_file"
				? "locate"
		...
		const answerPoints = (
			answerTemplate === "locate"
				...
				: answerTemplate === "feature_understanding"
					? [
							featurePacks[0]
								? `Feature pack: ${featurePacks[0].title}`
								: "Feature pack: none",
							featurePacks[0]?.summary
								? `Summary: ${featurePacks[0].summary}`
								: "Summary: none",
							featurePacks[0]?.primary_files?.length
								? `Primary files: ${featurePacks[0].primary_files.slice(0, 3).join(", ")}`
								: "Primary files: none",
							featurePacks[0]?.primary_symbols?.length
								? `Primary symbols: ${featurePacks[0].primary_symbols.slice(0, 3).join(", ")}`
		...
		return {
			...
			explainability: {
				...
				fallbacks: [
					...
					...(featurePacks.length === 0 && shouldAttachFeaturePack
						? ["feature_pack_unavailable_or_unresolved"]
						: []),
					...(overlay === null && retrievalPlan.attach_overlay
						? ["overlay_unavailable_or_unresolved"]
						: []),
				],
			},
			confidence: {
	...
	private buildDeveloperRetrievalPlan(
		...
		if (intent === "locate_file") {
			return {
				...
				attach_overlay: false,
				prefer_feature_primary: false,
				path_prefix_hint: this.buildLocatePathPrefixHint(
					parsed.relative_path || parsed.query_text,
				),
			};
		}
	...
	private parseProjectDeveloperQueryPayload(
		payload: ProjectDeveloperQueryPayload,
	): ProjectDeveloperQueryParsed {
		...
		const canonicalFromExplicit: Record<
			ProjectDeveloperQueryIntent,
			ProjectDeveloperQueryCanonicalIntent
		> = {
		...
		const canonical_intent = explicitIntent
			...
			: this.inferCanonicalIntentFromQuery({
					...
					hasFeatureSelector: Boolean(
						payload.feature_key || payload.feature_name,
					),
				});
		...
		const feature_key =
			payload.feature_key ||
			(payload.feature_name
				? this.tryResolveFeatureKeyInput(undefined, payload.feature_name)
				: null) ||
			(canonical_intent === "feature_lookup" && !query && inferredFeatureKey
				? inferredFeatureKey
				: null) ||
			(canonical_intent === "change_lookup"
				? "change_aware_impact"
		...
		if (!query_text) {
			throw new Error(
				"project.developer_query requires payload.query or deterministic selectors",
			);
		}
		...
		return {
			...
			...(routePath || routePaths[0]
				? { route_path: routePath || routePaths[0] }
				: {}),
			...(routePaths.length > 0 ? { route_paths: routePaths } : {}),
			...(trackerIssueKey || trackerIssueKeys[0]
				? { tracker_issue_key: trackerIssueKey || trackerIssueKeys[0] }
				: {}),
			...(taskId || taskIds[0] ? { task_id: taskId || taskIds[0] } : {}),
			...(taskTitle ? { task_title: taskTitle } : {}),
			...(trackerIssueKeys.length > 0
				? { tracker_issue_keys: trackerIssueKeys }
				: {}),
	...
	private inferCanonicalIntentFromQuery(input: {
		...
		if (
			/what breaks if|blast radius|affected|impact|impact analysis|change-aware|change aware|overlay|lookup/.test(
				lowered,
			)
		) {
			return "change_lookup";
		...
		if (
			/who handles|entrypoint for|where is .* implemented|route|endpoint|api\//.test(
				lowered,
			)
		) {
			return "locate_file";
		...
		if (
			input.hasFeatureSelector ||
			this.extractFeatureKeyFromQuery(input.query)
		) {
	...
	private tryResolveFeatureKeyInput(
		featureKey?: FeaturePackKey,
		featureName?: string,
	): FeaturePackKey | null {
		try {
	...
	private extractTrackerIssueKeys(query: string): string[] {
		...
		return Array.from(
			new Set(matches.map((item) => item.trim().toUpperCase()).filter(Boolean)),
		);
	}
	...
	private resolveFeatureKeyInput(
		featureKey?: FeaturePackKey,
		featureName?: string,
	): FeaturePackKey {
		...
		if (
			(has("onboarding") || has("registration") || has("index")) &&
			!has("retrieval")
		) {
	...
	private buildProjectOnboardingRegistrationIndexingPack(
		snapshot: ReturnType<
			SlotDB["getProjectFeaturePackProjectOnboardingIndexingSnapshot"]
		>,
		projectId: string,
		primaryAlias: string,
	): FeaturePackV1 {
		...
		const primaryFiles = this.rankPrimaryFiles(snapshot, [
			"src/commands/telegram-addproject-command.ts",
			"src/commands/project-telegram-onboarding.ts",
		]);
		...
		return {
			...
			summary: `Covers the cross-agent project setup flow from /asm_project_index onboarding through registry persistence, optional tracker linking, and index/reindex execution for project '${primaryAlias}'.`,
			primary_files: primaryFiles,
			primary_symbols: primarySymbols,
			flow_steps: [
				{
					...
					details:
						"Telegram /asm_project_index command and project onboarding helper collect repo, alias, Jira, and index-now intent.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/commands/telegram-addproject-command.ts",
				...
				},
				{
					step: 2,
					title:
						"Registration command resolves repo and persists registry state",
				...
				},
				{
					...
					details:
						"jira/github/other mapping is stored in project tracker mappings so later agents can navigate issue space consistently.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/core/usecases/default-memory-usecase-port.ts",
						"src/tools/project-tools.ts",
					]),
				...
				},
				{
					step: 4,
					title:
						"Index bootstrap or reindex updates searchable project context",
			...
			related_commits: this.collectRelatedCommitHints(snapshot, [
				"register",
				"index",
				"onboarding",
			]),
			...
			flow_steps: [
				{
					...
					details:
						"project.reindex_diff writes file_index_state, symbol_registry, and chunk_registry with active entries for changed files.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
						"src/core/usecases/default-memory-usecase-port.ts",
					]),
				...
				},
				{
					...
					details:
						"project.task_lineage_context assembles focus task, parent chain, related tasks, and touched symbols/files before ranking.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
						"src/core/usecases/default-memory-usecase-port.ts",
					]),
					related_symbols: [
						"project.task_lineage_context",
						"handleProjectTaskLineageContext",
					],
				},
				{
					step: 3,
					...
					details:
						"project.hybrid_search blends file/symbol/chunk/task candidates and supports debug candidate buckets for conformance checks.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
						"src/tools/project-tools.ts",
					]),
				...
				},
				{
					...
					details:
						"graph.code.upsert / graph.code.chain provide relation-level traversal for dependency/call chains when symbol-level context is needed.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/core/usecases/default-memory-usecase-port.ts",
						"src/tools/graph-tools.ts",
					]),
	...
	private buildHeartbeatHealthRuntimeIntegrityPack(
		...
		return {
			...
			primary_files: this.rankPrimaryFiles(snapshot, [], 10),
			...
			flow_steps: [
				{
					...
					details:
						"project registration_status + validation_status represent current readiness and integrity posture.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
					]),
				...
				},
				{
					...
					details:
						"Jira/GitHub tracker mappings are validated and persisted, reducing cross-agent drift in runtime operations.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/core/usecases/default-memory-usecase-port.ts",
					]),
	...
	private buildChangeAwareImpactPack(
		...
		return {
			...
			primary_files: this.rankPrimaryFiles(snapshot),
			primary_symbols: this.rankPrimarySymbols(snapshot, [
				"project.reindex_diff",
				"project.index_event",
				"project.index_watch_get",
			...
			flow_steps: [
				{
					...
					details:
						"project.reindex_diff (or project.index_event) accepts changed/deleted files and updates index/watch state.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
						"src/core/usecases/default-memory-usecase-port.ts",
					]),
				...
				},
				{
					...
					details:
						"project.task_registry_upsert tracks files_touched/symbols_touched/related tasks to enrich later impact analysis.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
					]),
					related_symbols: [
				...
				},
				{
					...
					details:
						"project.hybrid_search combines changed files/symbols and lineage context to return practical impact candidates.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
						"src/tools/project-tools.ts",
					]),
	...
	private buildPostEntryReviewDecisionSupportPack(
		...
		return {
			...
			primary_files: this.rankPrimaryFiles(
				snapshot,
				matchingSymbols.map((item) => item.relative_path).filter(Boolean) as string[],
				10,
			),
			primary_symbols: this.rankPrimarySymbols(snapshot, [
				"project.task_registry_upsert",
			...
			flow_steps: [
				{
					...
					details:
						"task_registry entries store decision_notes, task_status, tracker key, and touched files/symbols to preserve post-entry context.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
					]),
				...
				},
				{
					...
					details:
						"project.task_lineage_context can reconstruct parent/related chain to explain why a post-entry action was taken.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/core/usecases/default-memory-usecase-port.ts",
					]),
				...
				},
				{
					...
					details:
						"project.hybrid_search ranks symbols/files/tasks so agents can consume review evidence with minimal manual browsing.",
					related_files: this.projectLocalEvidenceFiles(snapshot, [
						"src/db/slot-db.ts",
						"src/tools/project-tools.ts",
					]),
	...
	private rankPrimaryFiles(
		...
		const activeProjectFiles = this.getProjectActiveFileSet(snapshot);
		const preferredRank = new Map(
			preferred.map((file, index) => [file, index] as const),
		);
		return Array.from(activeProjectFiles)
			.sort((a, b) => {
				const rankA = preferredRank.has(a)
					? preferredRank.get(a)!
					: Number.MAX_SAFE_INTEGER;
				const rankB = preferredRank.has(b)
					? preferredRank.get(b)!
					: Number.MAX_SAFE_INTEGER;
	...
	private rankPrimarySymbols(
		...
		const snapshotSymbols = Array.from(
			new Set(
				snapshot.recent_symbols
					.flatMap((item) => [item.symbol_fqn, item.symbol_name])
					.filter(Boolean) as string[],
			),
		);
		const preferredRank = new Map(
			preferred.map((symbol, index) => [symbol, index] as const),
		);
	...
	private projectLocalEvidenceFiles(
		snapshot: ReturnType<
			SlotDB["getProjectFeaturePackProjectOnboardingIndexingSnapshot"]
		>,
		candidates: string[],
		...
		return Array.from(
			new Set(
				candidates
					...
					.filter((path) => Boolean(path) && activeProjectFiles.has(path)),
			),
		).slice(0, limit);
	}

	private getProjectActiveFileSet(
		snapshot: ReturnType<
			SlotDB["getProjectFeaturePackProjectOnboardingIndexingSnapshot"]
		>,
	): Set<string> {
	...
	private buildEvidenceOrdered(
		...
		const jiraMapping =
			snapshot.tracker_mappings.find((item) => item.tracker_type === "jira") ||
			snapshot.tracker_mappings[0] ||
			null;

		return [
			{
				type: "project",
				ref: snapshot.project.project_id,
				note: snapshot.project.project_name,
			},
			...snapshot.aliases.slice(0, 3).map((item) => ({
				type: "project" as const,
				ref: item.project_alias,
				note: item.is_primary === 1 ? "primary_alias" : "alias",
			})),
			...(options.includeRegistration && registrationState
				? [
						{
							type: "registration" as const,
							ref: registrationState.registration_status,
							note: registrationState.validation_status,
						},
					]
				: []),
				...
				? [
						{
							...
							ref: jiraMapping.tracker_type,
							note:
								jiraMapping.tracker_space_key ||
								jiraMapping.default_epic_key ||
								undefined,
						},
					]
				: []),
			...snapshot.recent_index_runs
				.slice(0, options.includeIndexRuns)
				.map((item) => ({
					type: "index" as const,
					ref: item.run_id,
					note: `${item.trigger_type}:${item.state}`,
				})),
			...snapshot.recent_tasks.slice(0, options.includeTasks).map((item) => ({
				type: "task" as const,
				ref: item.tracker_issue_key || item.task_id,
				note: item.task_title,
			})),
			...snapshot.recent_files
				.slice(0, options.includeFiles)
				.map((item) => ({ type: "file" as const, ref: item.relative_path })),
			...snapshot.recent_symbols
				.slice(0, options.includeSymbols)
				.map((item) => ({
					type: "symbol" as const,
					ref: item.symbol_fqn || item.symbol_name,
					note: item.relative_path,
				})),
		];
	}

	private handleGraphEntityGet(
		payload: GraphEntityGetPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);

		if (payload.id) {
			return this.slotDb.graph.getEntity(
				identity.userId,
				identity.agentId,
				payload.id,
			);
	...
	private handleGraphEntitySet(
		...
		if (!payload.name || !payload.type) {
			throw new Error(
				"graph.entity.set requires payload.name and payload.type",
			);
		}

		if (payload.id) {
			const updated = this.slotDb.graph.updateEntity(
				identity.userId,
				identity.agentId,
				payload.id,
				{
	...
	private handleGraphRelAdd(
		...
		if (!payload.source_id || !payload.target_id || !payload.relation_type) {
			throw new Error(
				"graph.rel.add requires source_id, target_id, relation_type",
			);
		}

		const source = this.slotDb.graph.getEntity(
			identity.userId,
			identity.agentId,
			payload.source_id,
		);
		if (!source)
			throw new Error(`Source entity '${payload.source_id}' not found`);

		const target = this.slotDb.graph.getEntity(
			identity.userId,
			identity.agentId,
			payload.target_id,
		);
		if (!target)
			throw new Error(`Target entity '${payload.target_id}' not found`);

		return this.slotDb.graph.createRelationship(
			identity.userId,
			identity.agentId,
			{
	...
	private handleGraphRelRemove(
		...
		if (payload.id) {
			return {
				deleted: this.slotDb.graph.deleteRelationship(
					identity.userId,
					identity.agentId,
					payload.id,
				),
		...
		if (!payload.source_id || !payload.target_id || !payload.relation_type) {
			throw new Error(
				"graph.rel.remove requires id OR source_id + target_id + relation_type",
			);
		}

		const rels = this.slotDb.graph.getRelationships(
			identity.userId,
			identity.agentId,
			payload.source_id,
			"outgoing",
		...
		const rel = rels.find(
			(item) =>
				item.target_entity_id === payload.target_id &&
	...
	private async handleMemoryCapture(
		payload: Record<string, unknown>,
		req: CoreRequestEnvelope<unknown>,
	) {
		if (!this.semanticUseCase) {
			throw new Error(
				"memory.capture is not available: semantic runtime dependencies are not wired",
			);
		}
	...
	private async handleMemorySearch(
		...
		if (!this.semanticUseCase) {
			throw new Error(
				"memory.search is not available: semantic runtime dependencies are not wired",
			);
		}
	...
	private handleGraphSearch(
		...
		const traversed = this.slotDb.graph.traverseGraph(
			identity.userId,
			identity.agentId,
			payload.entity_id,
			depth,
		...
		return {
			...
			relationships: traversed.relationships.filter(
				(rel) => rel.relation_type === payload.relation_type,
			),
		};
	}

	private handleGraphCodeUpsert(
		payload: GraphCodeUpsertPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
		...
		for (const node of nodes) {
			if (
				!node.node_id ||
				!isUniversalGraphNodeType(node.node_type) ||
				!node.name
			) {
				throw new Error(
					"graph.code.upsert node invalid: require node_id, node_type, name",
				);
			}
			upsertUniversalGraphNode(
				this.slotDb.graph,
		...
		for (const relation of relations) {
			if (
				...
				throw new Error(
					"graph.code.upsert relation invalid: require source_node_id, target_node_id, relation_type, provenance",
				);
			}
			...
			if (!source) {
				throw new Error(
					`graph.code.upsert relation source '${relation.source_node_id}' not found`,
				);
			}
			const target = this.slotDb.graph.getEntity(
				identity.userId,
			...
			if (!target) {
				throw new Error(
					`graph.code.upsert relation target '${relation.target_node_id}' not found`,
				);
			}
	...
	private handleGraphCodeChain(
		payload: GraphCodeChainPayload,
		req: CoreRequestEnvelope<unknown>,
	) {
		const identity = normalizePrivateIdentity(req.context);
