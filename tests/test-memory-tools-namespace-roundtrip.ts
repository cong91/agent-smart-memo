...
class MockQdrant {
	...
	async search(
		...
		const matched = this.points.filter((p) =>
			this.matchesFilter(p.payload, filter),
		);
...
async function main() {
	...
	const memoryStore = createMemoryStoreTool(
		qdrant as any,
		embedding as any,
		dedupe,
	...
	await test("store normalizes alias namespace 'assistant' -> canonical", async () => {
		...
		assertEqual(
			saved.payload.memory_scope,
			"agent",
			"working_memory should map to memory_scope=agent",
		);
		assertEqual(
			saved.payload.memory_type,
			"episodic_trace",
			"working_memory should map to memory_type=episodic_trace",
		);
		assertEqual(
			saved.payload.promotion_state,
			"raw",
			"new captures should default promotion_state=raw",
		);
		assert(
			typeof saved.payload.confidence === "number",
			"confidence should be present in payload",
		);
	...
	await test("search normalizes alias namespace 'assistant' -> canonical filter", async () => {
		...
		assertEqual(
			nsCondition?.match?.value,
			"agent.assistant.working_memory",
			"search namespace filter must be canonical",
		);

		const text = res.content?.[0]?.text || "";
		assert(
			String(text).includes("Found"),
		...
		assert(
			details?.toolResult?.text,
			"memory_search details.toolResult.text must exist",
		);
	});
	...
	await test("legacy/shared namespace project_context maps to shared.project_context (store->search roundtrip)", async () => {
		...
		assertEqual(
			saved.payload.namespace,
			"shared.project_context",
			"project_context must normalize to shared.project_context",
		);
		assertEqual(
			saved.payload.memory_scope,
			"project",
			"shared.project_context should map memory_scope=project",
		);
		assertEqual(
			saved.payload.memory_type,
			"task_context",
			"shared.project_context should map memory_type=task_context",
		);
		...
		assertEqual(
			nsCondition?.match?.value,
			"shared.project_context",
			"legacy namespace search filter must map to shared.project_context",
		);
		assert(
			String(searchRes.content?.[0]?.text || "").includes("Found"),
			"legacy namespace roundtrip should find memory",
		);
	});
	...
	await test("backward compatibility: search still returns legacy payload without v2 metadata fields", async () => {
		qdrant.points.push({
			id: "legacy-no-v2-fields",
			vector: [0.1, 0.2, 0.3, 0.4],
			payload: {
				text: "legacy payload record",
				namespace: "agent.assistant.working_memory",
		...
		const res = await memorySearch.execute("t8", {
		...
		assert(
			res.isError !== true,
			"legacy payload without v2 metadata should still be searchable",
		);
		assert(
			String(res.content?.[0]?.text || "").includes("Found"),
			"search should still return found message for legacy payload",
		);
	});

	await test("sessionMode=soft does not inject hard session filter (default soft semantics)", async () => {
		const res = await memorySearch.execute("t9", {
			query: "soft session mode",
			namespace: "assistant" as any,
			agentId: "assistant",
		...
		const must = (qdrant.lastSearchFilter as any)?.must || [];
		const sessionCondition = must.find((m: any) => m?.key === "sessionId");
		assert(!sessionCondition, "soft mode must not add sessionId hard filter");
	});

	await test("sessionMode=strict injects hard session filter", async () => {
		const res = await memorySearch.execute("t10", {
			query: "strict session mode",
			namespace: "assistant" as any,
			agentId: "assistant",
		...
		const sessionCondition = must.find((m: any) => m?.key === "sessionId");
		assertEqual(
			sessionCondition?.match?.value,
			"session-strict-1",
			"strict mode must add sessionId hard filter",
		);
	});
