...
async function testReadyIsNonBlockingAndEmbedDetailedWorks() {
	...
	try {
		...
		const embedded = await withTimeout(
			client.embedDetailed("hello from nohang test"),
			1500,
			"embedDetailed",
		);
		assert(
			Array.isArray(embedded.vector) && embedded.vector.length === 4,
			"embedDetailed vector invalid",
		);
		assert(
			embedded.metadata.embedding_fallback_hash === false,
			"embedDetailed should not fallback hash",
		);
...
async function testMemoryToolsDoNotHang() {
	...
	try {
		...
		const storeRes = await withTimeout(
			memoryStore.execute("tc-store", {
				text: "Fix deadlock memory tools",
				agentId: "assistant",
			}),
			1500,
			"memory_store",
		...
		assert(
			...
			`memory_store failed: ${(storeRes as any).details?.toolResult?.text}`,
		);

		const searchRes = await withTimeout(
			memorySearch.execute("tc-search", {
				query: "deadlock memory",
				agentId: "assistant",
				minScore: 0.1,
			}),
			1500,
		...
		assert(
			...
			`memory_search failed: ${(searchRes as any).details?.toolResult?.text}`,
		);
		const text = (searchRes.content?.[0]?.text || "") as string;
		assert(
			text.includes("Found") || text.includes("No relevant memories"),
			"memory_search response not valid",
		);
	} finally {
		(global as any).fetch = originalFetch;
	}
