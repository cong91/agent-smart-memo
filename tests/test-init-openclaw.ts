...
test("resolveOpenClawConfigPath prefers OPENCLAW_STATE_DIR/openclaw.json", () => {
	const resolved = mod.resolveOpenClawConfigPath({
		HOME: ROOT,
	...
	assertEqual(
		resolved,
		configPath,
		"should resolve config path from OPENCLAW_STATE_DIR",
	);
});
...
test("buildPatchedConfig merges plugin block without dropping unrelated fields", () => {
	...
	assert(
		next.some_unrelated_top_level?.keep_me === true,
		"unrelated top-level field must be preserved",
	);
	assert(
		next.plugins.entries["other-plugin"].enabled === true,
		"existing plugin entries must remain",
	);
	assert(
		next.plugins.allow.includes("other-plugin"),
		"existing allow item must remain",
	);
	assert(
		next.plugins.allow.includes("agent-smart-memo"),
		"agent-smart-memo must be added to plugins.allow",
	);
	assertEqual(
		next.plugins.slots.memory,
		"agent-smart-memo",
		"plugins.slots.memory should map to agent-smart-memo",
	);
	assert(
		Array.isArray(next.channels?.telegram?.customCommands),
		"telegram customCommands should be created",
	);
	assert(
		next.channels.telegram.customCommands.some(
			(item: any) => item.command === "asm_project_index",
		),
	...
	assert(
		...
		"agent-smart-memo entry should be enabled",
	);
	assertEqual(
		entry.config.projectWorkspaceRoot,
		join(stateDir, "projects"),
		"openclaw entry should persist projectWorkspaceRoot",
	);
	assertEqual(
		entry.config.slotDbDir,
		join(stateDir, "agent-memo"),
		"openclaw entry should persist slotDbDir",
	);
	assertEqual(
		entry.config.wikiDir,
		join(stateDir, "wiki"),
		"openclaw entry should persist wikiDir",
	);
	assertEqual(
		Object.keys(entry.config).sort(),
		["projectWorkspaceRoot", "slotDbDir", "wikiDir"],
		"openclaw entry config should only expose the required 3-field runtime contract",
	);
});
...
test("buildPatchedConfig keeps single-account telegram merge at channels.telegram.customCommands", () => {
	...
	const commands = (next.channels.telegram.customCommands || []).map(
		(item: any) => item.command,
	);
	assertEqual(
		commands,
		["legacy", "asm_project_index"],
		"single-account merge should preserve + dedupe at root telegram customCommands",
	);
});
...
test("buildPatchedConfig fans out onboarding commands to enabled telegram accounts in multi-account mode", () => {
			...
			telegram: {
				accounts: {
					ops: {
						enabled: true,
						customCommands: [
							{ command: "legacyops", description: "Legacy ops" },
						],
					},
					growth: {
						enabled: true,
					...
					disabled: {
						enabled: false,
						customCommands: [
							{ command: "legacydisabled", description: "Disabled" },
						],
					},
				},
			},
	...
	const opsCommands = (
		next.channels.telegram.accounts.ops.customCommands || []
	).map((item: any) => item.command);
	const growthCommands = (
		next.channels.telegram.accounts.growth.customCommands || []
	).map((item: any) => item.command);
	const disabledCommands = (
		next.channels.telegram.accounts.disabled.customCommands || []
	).map((item: any) => item.command);

	assertEqual(
		opsCommands,
		["legacyops", "asm_project_index"],
		"enabled account ops should receive /asm_project_index with preserve+dedupe",
	);
	assertEqual(
		growthCommands,
		["asm_project_index"],
		"enabled account with existing /asm_project_index should be deduped",
	);
	assertEqual(
		disabledCommands,
		["legacydisabled"],
		"disabled account should remain unchanged",
	);
	assert(
		!Array.isArray(next.channels.telegram.customCommands),
...
test("buildSetupSummary classifies already configured / will add / will update", () => {
	const current = {
		channels: {
			telegram: {
				customCommands: [
					{ command: "asm_project_index", description: "Project onboarding" },
				],
			},
		},
		plugins: {
	...
	assert(
		summary.alreadyConfigured.includes(
			"plugins.allow includes agent-smart-memo",
		),
		"plugin allow should be already configured",
	);
	assert(
		summary.alreadyConfigured.includes(
			"plugins.entries.agent-smart-memo.config.projectWorkspaceRoot",
		),
		"projectWorkspaceRoot should be already configured",
	);
	assert(
		summary.alreadyConfigured.includes(
			"plugins.entries.agent-smart-memo.config.slotDbDir",
		),
		"slotDbDir should be already configured",
	);
	assert(
		summary.alreadyConfigured.includes(
			"plugins.entries.agent-smart-memo.config.wikiDir",
		),
		"wikiDir should be already configured",
	);
	assert(
		summary.willAdd.includes(
			"channels.telegram.customCommands includes /indexproject",
		),
...
test("formatSetupSummary renders required operator sections", () => {
	const output = mod.formatSetupSummary({
		...
		willUpdate: ["plugins.entries.agent-smart-memo.config.wikiDir"],
	});

	assert(
		output.includes("already configured"),
		"must include already configured section",
	);
...
test("buildBackupPath uses .bak timestamp suffix", () => {
	const sample = mod.buildBackupPath(
		configPath,
		new Date("2026-03-13T10:11:12Z"),
	);
...
test("runInitOpenClaw supports non-interactive auto-apply write path", async () => {
	...
	try {
		const result = await mod.runInitOpenClaw({
			env,
			interactive: false,
			autoApply: true,
		});
		...
		const commands =
			updated?.channels?.telegram?.customCommands?.map(
				(item: any) => item.command,
			) || [];

		assert(
			commands.includes("asm_project_index"),
			"auto-apply should include /asm_project_index in telegram customCommands",
		);
		assert(
			logs.some((line) => line.includes("Auto-apply mode enabled")),
			"should log auto-apply mode",
		);
	} finally {
